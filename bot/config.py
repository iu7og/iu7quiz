...
RULES_MSG = "🤯 Система оценивания зависит от следующих факторов:\n\n" \
    ...
    "💡 Каждый день Вы получаете некоторое количество новых вопросов. " \
    "В случае правильного ответа на какой либо из вопросов, " \
    ...
    "(такие вопросы задаются сразу после новых), нужно " \
...
HELP_MSG = "❓ Не знаете, что делать❓\n" \
    ...
    "6. Вызвать команду */stat*, чтобы получить свою статистику (кол-во правильных ответов," \
    " общее кол-во ответов, их процентное соотношение; самый быстрый и самый медленный ответы) 📈"

# Количество вопросов, добавляемое за день
QUESTION_PORTION = 3

# Количество дней, спустя которое бот начинает скучать по пользователю (сколько суток игнора).
MISS_DAYS = 2

MISS_MESSAGE = "🥺 Давно тебя не было в этом чате, я скучаю по твоим ответам :("
