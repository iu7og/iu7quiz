...
def answer_summary(stdnt: Student, qstn: Question, i=-1):
    ...
    return formula(1 - qstn.first_to_answer / qstn.total_answers, stdnt.data[qstn.day]['right'][i][0], stdnt.data[qstn.day]['right'][i][1], i + 1 + stdnt.data["wrong"] if i != -1 else len(stdnt.data['right']) + stdnt.data['wrong'], qstn.best_time_to_answer)
